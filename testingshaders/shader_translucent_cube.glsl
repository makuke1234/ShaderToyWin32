...
vec3 fcos(vec3 x) {
    ...
    //w*=0.; //test
    if((length(w)==0.)){vec3 tc=vec3(0.); for(int i=0;i<8;i++)tc+=cos(x+x*float(i-4)*(0.01*400./iResolution.y));return tc/8.;}
...
void main()
{
    ...
    vec2 uv = ((fragCoord + 0.5) * iResolution + o - 0.5 * iResolution.xy) / iResolution.x;
#else
    vec2 uv = ((fragCoord + 0.5) * iResolution - 0.5 * iResolution.xy) / iResolution.x;
#endif
...
#if defined(BG_ALPHA)||defined(ONLY_BOX)||defined(SHADOW_ALPHA)
    color.rgb=color.rgb*color.w+texture(iChannel0, fragCoord/iResolution.xy).rgb*(1.-color.w);
#endif
    //color=vec4(color.w);
